...
class SwipeRevealLayout : ViewGroup {
    ...
    private var mDelegate: SwipeRevealLayoutInterface? = null
    fun setSwipeRevealLayoutDelegate(listener: SwipeRevealLayoutInterface?) {
        mDelegate = listener
    }
    ...
    private fun init(
        ...
        if (attrs != null && context != null) {
            ...
            mDragEdge = a.getInteger(
                R.styleable.SwipeRevealLayout_dragEdge,
                DRAG_EDGE_LEFT
            )
            mMode = MODE_NORMAL
        ...
        mDragHelper?.run { setEdgeTrackingEnabled(ViewDragHelper.EDGE_ALL) }
...
// properly on RecyclerView or view that handles scroll gesture.
    ...
    private val mDragHelperCallback: ViewDragHelper.Callback = object : ViewDragHelper.Callback() {
            ...
            return when (mDragEdge) {
                DRAG_EDGE_RIGHT -> Math.max(
                    Math.min(
                        left,
                        mRectMainClose.left
                    ), mRectMainClose.left - mHorizontalRange
                )
        ...
        override fun onViewPositionChanged(
            ...
            isOpen = Math.abs(left) > MIN_CHECK_IS_OPEN
            ViewCompat.postInvalidateOnAnimation(this@SwipeRevealLayout)
        }
    }

    private fun swipeAllToDelete() {
        if (mDraggingBorder < -(mHorizontalRange / 2) && mDelegate != null) {
            mDelegate!!.dismissItem()
            return
        }
    }
    ...
    companion object {
        ...
        private const val MIN_CHECK_IS_OPEN = 10
    }
}